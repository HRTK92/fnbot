...
class PartyCommands(commands.Cog):
	...
	async def banner(self,
	                 ...
	                 banner_level: Optional[int] = None) -> None:
		await self.bot.party.me.set_banner(
		    icon=icon, color=colour, season_level=banner_level)

		await ctx.send(
		    f'Banner set to: {icon} with {colour} at level {banner_level}.')
	...
	@commands.command(
	    ...
	    "Example: !bp 100")
	async def bp(self, ctx: fortnitepy.ext.commands.Context,
	             tier: int) -> None:
	...
	@commands.command(
	    ...
	    "Example: !level 999")
	async def level(self, ctx: fortnitepy.ext.commands.Context,
	                banner_level: int) -> None:
		await self.bot.party.me.set_banner(season_level=banner_level)
	...
	@commands.command(
	    description=
	    "[Party] Sends message to party chat with the given content.",
	    help="Sends message to party chat with the given content.\n"
	    "Example: !echo i cant fix the fucking public lobby bots")
	async def echo(self, ctx: fortnitepy.ext.commands.Context, *,
	               content: str) -> None:
	...
	@commands.command(
	    ...
	    "Example: !kick Cxnyaa")
	async def kick(self, ctx: fortnitepy.ext.commands.Context, *,
	               epic_username: str) -> None:
		user = await self.bot.fetch_user(epic_username)
		member = self.bot.party.members.get(user.id)

		if member is None:
			await ctx.send(
			    "Failed to find that user, are you sure they're in the party?")
		else:
			try:
				await member.kick()
				...
				await ctx.send(
				    f"Failed to kick {member.display_name}, as I'm not party leader."
				)
				print(
				    crayons.red(
	...
	@commands.command(
	    aliases=['unhide'],
	    description=
	    "[Party] Promotes the defined user to party leader. If friend is left blank, "
	    "the message author will be used.",
	    help=
	    "Promotes the defined user to party leader. If friend is left blank, the message author will be used.\n"
	    "Example: !promote Terbau")
	async def promote(self,
	                  ctx: fortnitepy.ext.commands.Context,
	                  *,
	                  epic_username: Optional[str] = None) -> None:
		...
		if member is None:
			...
			try:
				...
				print(
				    self.bot.message % f"Promoted user: {member.display_name}")
			except fortnitepy.errors.Forbidden:
				await ctx.send(
				    f"Failed topromote {member.display_name}, as I'm not party leader."
				)
				print(
				    crayons.red(
				        self.bot.message % f"[ERROR] "
	...
	async def playlist_id(self, ctx: fortnitepy.ext.commands.Context,
	                      playlist_: str) -> None:
		try:
	...
	async def privacy(self, ctx: fortnitepy.ext.commands.Context,
	                  privacy_type: str) -> None:
		try:
			if privacy_type.lower() == 'public':
				await self.bot.party.set_privacy(
			...
			print(self.bot.message %
			      f'Party privacy set to {self.bot.party.privacy}.')

		except fortnitepy.errors.Forbidden:
			await ctx.send(
			    f"Failed to set party privacy to {privacy_type}, as I'm not party leader."
			)
			print(
			    crayons.red(
			        self.bot.message % f"[ERROR] "
	...
	async def matchmakingcode(self, ctx: fortnitepy.ext.commands.Context, *,
	                          custom_matchmaking_key: str) -> None:
		await self.bot.party.set_custom_key(key=custom_matchmaking_key)

		await ctx.send(
		    f'Custom matchmaking code set to: {custom_matchmaking_key}')

	@commands.dm_only()
	@commands.command(
	    description=
	    "[Party] Sets the client to the \"In Match\" state. If the first argument is 'progressive', "
	...
	async def match(self,
		...
		if players == 'progressive':
			...
			await self.bot.party.me.set_in_match(
			    players_left=100, started_at=match_time)

			while (100 >= self.bot.party.me.match_players_left > 0
			       and self.bot.party.me.in_match()):
				await self.bot.party.me.set_in_match(
				    players_left=self.bot.party.me.match_players_left -
				    py_random.randint(3, 6),
			...
			await self.bot.party.me.set_in_match(
			    ...
			    datetime.timedelta(minutes=match_time))

			await ctx.send(
			    f'Set state to in-game in a match with {players} players.'
	...
	async def lobby(self, ctx: fortnitepy.ext.commands.Context) -> None:
		if self.bot.default_party_member_config.cls == fortnitepy.JustChattingClientPartyMember:
			...
			await ctx.send(
			    'Removed state of Just Chattin\'. Now attempting to rejoin party.'
			)
			...
			try:
				...
				await ctx.send(
				    'Party not found, are you sure Fortnite is open?')
	...
	@commands.command(
	    description=
	    "[Party] Joins the party of the defined friend. If friend is left blank, "
	    "the message author will be used.",
	...
	async def join(self,
		...
		if isinstance(epic_friend, fortnitepy.Friend):
			try:
				...
				await ctx.send(
				    f'Joined the party of {epic_friend.display_name}.')
			except fortnitepy.errors.Forbidden:
	...
	async def friend(self, ctx: fortnitepy.ext.commands.Context, *,
	                 epic_username: str) -> None:
		if data['friend_accept']:
			await ctx.send(
			    'All friend requests will be accepted so there is no need to add manually.'
			)
			print(
			    self.bot.message %
			    f'!friend command ignored as friend requests will be accepted '
			...
			if user is not None:
				...
				await ctx.send(
				    f'Sent/accepted friend request to/from {user.display_name}.'
				)
				print(
				    self.bot.message %
				    f'Sent/accepted friend request to/from {user.display_name}.'
				...
				print(
				    crayons.red(
				        self.bot.message %
				        f"[ERROR] Failed to find a user with the name {epic_username}."
				    ))

	@commands.dm_only()
	@commands.command(
	    description=
	    "[Party] Sets the lobbies selected playlist using playlist name.",
	    help="Sets the lobbies selected playlist using playlist name.\n"
	    "Example: !playlist Food Fight")
	async def playlist(self, ctx: fortnitepy.ext.commands.Context, *,
	                   playlist_name: str) -> None:
		try:
			scuffedapi_playlist_id = await self.get_playlist(playlist_name)

			if scuffedapi_playlist_id is not None:
				await self.bot.party.set_playlist(
				    playlist=scuffedapi_playlist_id)
				await ctx.send(f'Playlist set to {scuffedapi_playlist_id}.')
				print(self.bot.message %
				      f'Playlist set to {scuffedapi_playlist_id}.')
	...
	@commands.command(
	    name="invite",
	    description=
	    "[Party] Invites the defined friend to the party. If friend is left blank, "
	    "the message author will be used.",
	...
	async def _invite(self,
		...
		if isinstance(epic_friend, fortnitepy.Friend):
			try:
				...
				await ctx.send(
				    f'Invited {epic_friend.display_name} to the party.')
				print(
				    self.bot.message %
				    f"[ERROR] Invited {epic_friend.display_name} to the party."
				)
			except fortnitepy.errors.PartyError:
				await ctx.send(
				    'Failed to invite friend as they are either already in the party or it is full.'
				)
				print(
				    crayons.red(
				        self.bot.message % f"[ERROR] "
	...
	@commands.command(
	    description=
	    "[Party] Hides everyone in the party except for the bot but if a player is specified, "
	    "that specific player will be hidden.",
	    help="Hides members of the party.\n"
	    "Example: !hide")
	async def hide(self,
	               ctx: fortnitepy.ext.commands.Context,
	               party_member: Optional[str] = None) -> None:
		if self.bot.party.me.leader:
			if party_member is not None:
				...
				await self.bot.set_and_update_party_prop(
				    'Default:RawSquadAssignments_j', {
				        'RawSquadAssignments': [{
				            'memberId': self.bot.user.id,
				            'absoluteMemberIdx': 1
				        }]
				    })

				await ctx.send(
				    'Hid everyone in the party. Use !unhide if you want to unhide everyone.'
	...
	async def justchattin(self, ctx: fortnitepy.ext.commands.Context) -> None:
		...
		await ctx.send(
		    'Set state to Just Chattin\'. Now attempting to rejoin party.'
		    '\nUse the command: !lobby to revert back to normal.')
